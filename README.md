...
> **이름이 두 벌인 이유**: 웹 UI의 탭 이름(`지식화 처리`·`AI 탐색`·`품질 진단`)은 **화면용 표기**입니다.
> 볼트 규칙·스킬·CLI는 계속 `ingest` / `query` / `lint`를 씁니다.
> 어느 이름으로 부르든 **같은 규칙 파일을 읽어 도는 같은 연산**입니다.
...
| `python-multipart` | 「데이터 등록」 탭의 파일 업로드 |
| `markdown` | 「지식베이스」 탭의 읽기 전용 렌더 |
...
왼쪽부터 화면에 나오는 순서 그대로입니다.
**연산** 열은 그 탭이 [3대 핵심 연산](#2-3대-핵심-연산-core-operations) 중 무엇을 실행하는지입니다 —
탭 이름은 화면용이고, 근본 동작은 스킬·CLI와 같습니다.

| 탭 | 연산 | 하는 일 |
|---|---|---|
| **대시보드** | — | 페이지 수, 연구 축 분포, 마지막 진단 결과, 다음에 만들 페이지 |
| **지식베이스** | — | 위키·원본 읽기 전용 뷰어 + 본문 검색. 목록 폭은 끌어서 조절, 편집은 `obsidian://`로 넘김 |
| **AI 탐색** | `query` | 위키를 근거로 질의. 토큰 스트리밍·추론 과정 표시·중단·출처 드로어 |
| **데이터 등록** | — | Zotero 라이브러리에서 가져오거나 URL·파일로 `raw/`에 추가 |
| **지식화 처리** | `ingest` | 대기 중인 자료를 확인하고 실행 → **적용 전에 diff를 먼저 보여줌**. 특히 *지워지는 줄*을 따로 뽑아줍니다 |
| **품질 진단** | `lint` | 진단 실행 및 심각도별 결과. 예외 처리는 `lint-ignore.json`에 파일로 남음 |
